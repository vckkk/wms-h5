...
import { scanOrder, scanPurchase, scanSku } from '@/server/scanPick';
import { getRealStr } from '@/utils';
import { Button, SearchBar, SpinLoading, Toast } from 'antd-mobile';
...
const Scan = () => {
  ...
  const getOrder = (params?: string) => {
    //fetch 后清空value
    setLoading(true);
    scanOrder({ order_name: params || orderId })
      .then((res: any) => {
        setOrderId('');
        if (res.success === true) {
  ...
  const getSkuInfo = (code: string, name?: string) => {
    ...
    setLoading(true);
    if (isPurchase) {
      scanPurchase({ purchase_index: code })
        .then((res: any) => {
          if (res.success === true) {
    ...
    scanSku({ order_index: code, order_name: orderName.order_name || name })
      .then((res: any) => {
        if (res.success === true) {
          setSkuInfo(res?.result);
        } else {
  ...
  return (
    <div className={styles.content}>
      {step === 1 && (
        <SearchBar
          placeholder={orderName?.order_name ? '请扫描商品' : '请扫描订单号'}
          style={{ '--height': '32px' }}
          searchIcon={<ScanningOutline onClick={onFocusHandle} />}
          value={orderId}
          onChange={setOrderId}
          ref={searchRef}
      ...
      {orderName.order_name && step === 1 && (
        <div className={styles.orderName}>
          ...
          <div>
            履约进度: {orderName.approved_count || 0}/{orderName.total_rows || 0}
          </div>
        </div>
      )}
      <div>
      ...
      {orderName.order_name && step === 1 && (
        <Button className={styles.btnBack} size="large" color="primary" onClick={() => setOrderName({})} block>
          返回
        </Button>
      )}
    </div>
